...
class SystemErrorList:
    ...
    errors[Error.USER_NOT_FOUND.value] = SystemError(
        ...
        "UserId [%s] does not exist",
        status.HTTP_400_BAD_REQUEST,
    )
    errors[Error.USER_EMAIL_ALREADY_EXIST.value] = SystemError(
        Error.USER_EMAIL_ALREADY_EXIST.value,
        "Email [%s] already exist",
        status.HTTP_400_BAD_REQUEST,
    )
    errors[Error.USER_TWO_PASSWORD_DOES_NOT_EQUAL.value] = SystemError(
        Error.USER_TWO_PASSWORD_DOES_NOT_EQUAL.value,
        "Password1 and password2 does not matched",
        status.HTTP_400_BAD_REQUEST,
    )
    errors[Error.GROUP_NOT_FOUND.value] = SystemError(
        Error.GROUP_NOT_FOUND.value,
        "GroupId [%s] does not exist",
        status.HTTP_400_BAD_REQUEST,
    )
    errors[Error.AUTHORIZATION_NOT_FOUND.value] = SystemError(
        Error.AUTHORIZATION_NOT_FOUND.value,
        "AuthorizationId [%s] does not exist",
        status.HTTP_400_BAD_REQUEST,
    )
    ...
    errors[Error.USER_TWO_PASSWORD_SHOULD_BE_SENT.value] = SystemError(
        Error.USER_TWO_PASSWORD_SHOULD_BE_SENT.value,
        "NEED TWO PASSWORD",
    ...
    errors[Error.PAGE_URL_EXIST.value] = SystemError(
        Error.PAGE_URL_EXIST.value,
        "Page url [%s] already exist in service",
        status.HTTP_400_BAD_REQUEST,
    )
...
class SystemException(APIException):
    ...
    def __init__(self, SystemError, arg=None):
        ...
        default_detail = {"errorCode": SystemError.errorCode, "errorMessage": errorMsg}
        ...
        if response.data is not None:
            ...
            if "errorCode" in response.data:
                ...
                if "JSON parse error" in response.data["detail"]:
                    ...
                    SystemError = SystemErrorList.errors[
                        Error.AUTH_CREDENTIAL_REQUIRED.value
                    ]
