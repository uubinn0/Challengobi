...
class User(AbstractUser):
    ...
    nickname = models.CharField(
        max_length=100,
        unique=True,
        error_messages={"unique": _("이미 존재하는 닉네임입니다.")},
        verbose_name="닉네임",
        default="굴비",
    )
    ...
    career = models.PositiveSmallIntegerField(
        choices=CAREER_CHOICES, null=False, blank=False, default=1
    )
...
class UserChallengeCategory(models.Model):
    user = models.OneToOneField(
        User, on_delete=models.CASCADE, related_name="challenge_category"
    )  # 역참조
